  ...
  try {
    ...
    switch (service) {
      ...
      case "native-n": {
        ...
        const captchaToken = searchParams.c
        ...
        const sessionToken = jwt.sign(
          { userId: user.userid, iat: Math.floor(Date.now() / 1000) },
          process.env.SESSION_JWT_TOKEN!,
          { expiresIn: "2d" }
        );
      ...
      case "native-r": {
        ...
        const sessionToken = jwt.sign(
          { userId: userCheck.rows[0].id, iat: Math.floor(Date.now() / 1000)},
          process.env.SESSION_JWT_TOKEN!,
          { expiresIn: "2d" }
        );
